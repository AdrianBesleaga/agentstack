...
description: "Manage conversation state and history to build context-aware agents in Agent Stack"
---
...
1. **Access conversation history:** Use `RunContext` to set up an instance of the conversation history to store and load previous messages.
1. **Store incoming messages:** Use `await context.store(input)` to store the current user message in the conversation history.
...
1. **Store agent responses:** Use `await context.store(response)` to store your agent's responses for future conversation context.
...
- With `PlatformContextStore`, each `context.store()` call creates a persisted history item, so buffering prevents chunk-level history fragmentation.
